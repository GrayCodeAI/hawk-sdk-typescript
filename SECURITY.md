# Security Policy — hawk-sdk-typescript
...
1. Open a private [GitHub Security Advisory](https://github.com/GrayCodeAI/hawk-sdk-typescript/security/advisories/new), **or**
...
- **Dependency monitoring:** vulnerable dependencies are detected by
  `npm audit`, which runs on every CI build (see `.github/workflows/ci.yml`).
- **Static analysis:** `typescript` type-checking and linting are enforced
  in CI.
- **Vulnerability scanning:** `npm audit --audit-level=high` runs on every CI build.
- **Lockfiles:** dependencies are pinned in `package-lock.json`.
- **Reproducible builds:** the runtime build in `dist/` has zero dependencies.
...
- LLM provider services that hawk-sdk-typescript integrates with (report to the
  provider).
- Local filesystem misuse where an attacker already has shell access (out of
  threat model).

For hawk-sdk-typescript-specific threat-model notes, see the README.
